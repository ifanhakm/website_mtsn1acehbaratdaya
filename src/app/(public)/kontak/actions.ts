...
export async function sendContactEmail(formData: {
  ...
  try {
    ...
    const schoolEmail = process.env.NEXT_PUBLIC_SCHOOL_EMAIL || 'website.mtsn1abdya@gmail.com'

    // 2. Kirim email menggunakan Resend API
    const { data, error } = await resend.emails.send({
      from: 'Website MTsN 1 ABDYA <onboarding@resend.dev>', // Sender bawaan Resend Free Sandbox
      to: schoolEmail,
        ...
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 16px; background-color: #ffffff;">
          ...
          <div style="padding: 24px; color: #1f2937;">
            <p style="font-size: 14px; line-height: 1.6; margin-bottom: 20px; color: #4b5563;">
              Halo Tim Humas MTsN 1 Abdya, Anda menerima pesan baru dari pengunjung website sekolah dengan rincian sebagai berikut:
            </p>
